...
<html>
	<head>
		<meta charset="UTF-8">
		<title>Boulders Problems Climbed by Selected Climber</title>
		<link rel="stylesheet" type="text/css" href="style.css">
	</head>
	<body>
		<!-- fill table with data-->
		<table>
			<caption>Boulders Problems Climbed by Selected Climber</caption>
			<thead>
				<tr>
					<th>Name</th>
					<th>Grade</th>				
			...
			<tbody>
				...
				INNER JOIN location l ON bl.lid = l.id
				INNER JOIN boulder_climber bc ON bc.bid = b.id
				WHERE bc.cid = ?"))){
					echo "Prepare Failed: " . $stmt->errno . " " . $stmt->error;
				}
				
				if(!($stmt->bind_param("i",$_POST['filterBouldersClimbedByClimber']))){

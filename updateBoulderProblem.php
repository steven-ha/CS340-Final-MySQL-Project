...
<html>
	<head>
		<meta charset="UTF-8">
		<title>Update Boulder Data</title>
	...
	<body>
		...
		// boulder to update
		$updateID = $_POST['updateBoulderProblem'];	

		// get the previous data
		if(!($stmt = $mysqli->prepare("SELECT name, grade FROM boulder WHERE id=$updateID"))){
		...
		// if statement checks if the boulder will have new name or not
		if (!($_POST['updateName'] == ''))			
		{
			//update name
		...
		// if statement checks if the boulder will have new grade or not
		...
		//update the boulder
		if(!($stmt = $mysqli->prepare("UPDATE boulder SET name=?, grade=? WHERE id=$updateID"))){
			echo "Prepare Failed: " . $stmt->errno . " " . $stmt->error;
		}
		...
		if(!$stmt->execute()){
			...
			echo "Updated " . $stmt->affected_rows . " rows in boulder.";
		}

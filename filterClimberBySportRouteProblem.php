...
<html>
	<head>
		<meta charset="UTF-8">
		<title>Climbers by Selected Sport Route Problem</title>
		<link rel="stylesheet" type="text/css" href="style.css">
	</head>
	<body>
		<!-- fill table with data-->
		<table>
			<caption>Climbers by Selected Sport Route Problem</caption>
			<thead>
				<tr>
					<th>First Name</th>
					<th>Last Name</th>				
			...
			<tbody>
				<?php
				if(!($stmt = $mysqli->prepare("SELECT c.firstName, c.lastName, c.birthday, s.name, s.id, s.grade, l.name FROM sport_climber sc
				...
				if(!($stmt->bind_param("i",$_POST['filterClimberBySportRouteProblem']))){
				...
				if(!$stmt->bind_result($firstName, $lastName, $birthday, $sportName, $sportid, $sportGrade, $locationName)){
